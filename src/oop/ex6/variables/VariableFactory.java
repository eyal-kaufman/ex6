...
import oop.ex6.variables.exceptions.InvalidCastingException;
import oop.ex6.variables.exceptions.InvalidDeclarationException;
import oop.ex6.variables.exceptions.InvalidVariableNameException;
...
public class VariableFactory {
	 ...
	 * 2. it verify this variable name wasn't already declared in this line
	...
	private void parseDeclaration(String line) throws VariableException {
		...
		if (variables.length != 1 && this.type == null) {
			throw new InvalidCastingException();
		}
	...
	/**
	 * check if the given value is fit to the type in declaration if exist, or to the
		...
		return this.type.checkValueType(value) ||
				(Types.isVariableCasting(value) &&
						(this.getVariable(value) != null
								&& this.type.approvedType(Objects.requireNonNull(this.getVariable(value)).getType())
								&& Objects.requireNonNull(this.getVariable(value)).isInitialized()));
	}
	 ...
	 *
	 * @param modifier the extracted modifier
	 * @return true if it's fit, false else.
	 */
